...
const playlistPersonal = $(
  ".container__personal--wrapper-item .song-side__playlist--carousel.playlist"
);
const albumPersonal = $(
  ".container__personal--wrapper-item .song-side__playlist--carousel.album"
);
...
const carouselPlaylist = $(
  ".song-side__playlist .song-side__playlist--carousel"
);
...
getData = (api) => {
  ...
  //   })
  //   .catch((err) => {
  //     console.log("co loi getdata", err);
  //   });
  ...
  //       console.log("in du lieu request", typeof request.response);
  //     } else {
...
Promise.all([
  ...
  .then(
    ([songs, singers, playlists, videos, songsUSUK, songsEDM, ranksTable]) => {
      ...
      rankTableData = ranksTable; // gan du lieu promise tra ve vao mang rankTableData
    }
  )
  .then(() => app.start())
  .catch((err) => alert(err));
...
const app = {
        ...
        datasets: [
          {
            ...
            data: [
              860, 1140, 1060, 1060, 1070, 1110, 1330, 2210, 7830, 2478, 4000,
              4800,
            ],
            borderColor: "red",
            fill: false,
          ...
          },
          {
            ...
            data: [
              1600, 1700, 1700, 1900, 2000, 2700, 4000, 5000, 6000, 7000, 5000,
              6600,
            ],
            borderColor: "yellow",
            fill: false,
          ...
          },
          {
            ...
            data: [
              300, 700, 2000, 5000, 6000, 4000, 2000, 1000, 200, 100, 1200,
              2000,
            ],
            borderColor: "blue",
            fill: false,
    ...
    if (app.isPlayPersonalSong) {
      Array.from($$(".personal__song--list-item")).forEach((item) => {
        if (item.classList.contains("active")) {
          ...
          item.firstElementChild.firstElementChild.nextElementSibling.firstElementChild.style.display =
            "none";
          item.firstElementChild.firstElementChild.nextElementSibling.lastElementChild.style.filter =
            "brightness(100%)";
        }
      });
      const itemCurrent = $$(".personal__song--list-item")[app.currentIndex];
      itemCurrent.classList.add("active");
      const waveActiveSong =
        itemCurrent.firstElementChild.firstElementChild.nextElementSibling
          .firstElementChild;
      const thumbnailActiveSong =
        itemCurrent.firstElementChild.firstElementChild.nextElementSibling
          .lastElementChild;
      ...
      app.activeItemWhenChangingPlaylist(
        $$(" .menu-side__song.us-uk .menu-side__song-item")
      );
    } else if (app.isPlayEDM) {
      app.activeItemWhenChangingPlaylist(
        $$(" .menu-side__song.edm .menu-side__song-item")
      );
    } else {
  ...
  removeSongItemActive: (elementNodeList) => {
    Array.from(elementNodeList).forEach((item) => {
      if (item.classList.contains("active")) {
        ...
        const thumbnail =
          item.firstElementChild.nextElementSibling.lastElementChild;
  ...
  activeItemWhenChangingPersonal: (listItemSong) => {
    ...
    const waveActiveSong =
      listItemSong[app.currentIndex].firstElementChild.firstElementChild
  ...
  activeItemWhenChangingPlaylist: (listItemSong) => {
    ...
    const iconElement =
      listItemSong[app.currentIndex].lastElementChild.firstElementChild;
    ...
    audio.play();
    const waveActiveSong =
      listItemSong[app.currentIndex].firstElementChild.nextElementSibling
        .firstElementChild;
    const imgActiveSong =
      listItemSong[app.currentIndex].firstElementChild.nextElementSibling
        .lastElementChild;
  ...
  selectTheme: () => {
    themeModal.onclick = (e) => {
      ...
      const indexOfType = e.target.closest(".content__item--theme").dataset
        .index;
  ...
  selectSongPlaylist: () => {
    menuSongPlaylist.onclick = (e) => {
      ...
      const wavePlaylist =
        songItem.firstElementChild.nextElementSibling.firstElementChild;
      const thumbnail =
        songItem.firstElementChild.nextElementSibling.lastElementChild;
  ...
  selectSongPlaylistUSUK: () => {
    menuSongPlaylistUSUK.onclick = (e) => {
      (app.isPlayPersonalSong = false), (app.isPlayUSUK = true);
      app.isPlayEDM = false;
      Array.from($$(".personal__song--list-item")).forEach((item) => {
        if (item.classList.contains("active")) {
          item.classList.remove("active");
  ...
  selectSongPlaylistEDM: () => {
    menuSongPlaylistEDM.onclick = (e) => {
      (app.isPlayPersonalSong = false), (app.isPlayUSUK = false);
      app.isPlayEDM = true;
      Array.from($$(".personal__song--list-item")).forEach((item) => {
        if (item.classList.contains("active")) {
          item.classList.remove("active");
  ...
  selectSongPersonal: () => {
    personalSongPlaylist.onclick = (e) => {
      ...
      songItem.classList.add("active");
      songItem.firstElementChild.firstElementChild.nextElementSibling.firstElementChild.style.display =
        "flex";
      songItem.firstElementChild.firstElementChild.nextElementSibling.lastElementChild.style.filter =
        "brightness(50%)";
      app.currentIndex = songItem.getAttribute("data-index");
      app.loadCurrentSong();
      audio.play();
      ...
      if (!app.isPlaying && audio.paused) {
        ...
        if (!app.isPlayPersonalSong) {
          const wavePlaylist =
            listItemSong[app.currentIndex].firstElementChild.nextElementSibling
              .firstElementChild;
          const thumbnailPlaylist =
            listItemSong[app.currentIndex].firstElementChild.nextElementSibling
              .lastElementChild;
          const iconPlaylist =
            listItemSong[app.currentIndex].lastElementChild.firstElementChild;
          iconPlaylist.classList.replace(
            "bi-play-circle-fill",
            "bi-pause-circle-fill"
          );
          wavePlaylist.style.display = "flex";
          ...
          const wavePersonal =
            listItemSongPersonal[app.currentIndex].firstElementChild
        ...
        btnPlayAll.firstElementChild.classList.replace(
          "bi-pause-fill",
          "bi-play-fill"
        );
      }
    ...
    btnNext.onclick = () => {
      if (app.isRandom) {
        ...
        if (app.isPlayPersonalSong) {
          ...
          const itemCurrent = $$(".personal__song--list-item")[
            app.currentIndex
          ];
          itemCurrent.classList.add("active");
          itemCurrent.firstElementChild.firstElementChild.nextElementSibling.firstElementChild.style.display =
            "flex";
          itemCurrent.firstElementChild.firstElementChild.nextElementSibling.lastElementChild.style.filter =
            "brightness(50%)";
          app.loadCurrentSong();
          audio.play();
        } else if (app.isPlayUSUK) {
...
rightScrollPlaylistPersonal.onclick = () =>
  (playlistPersonal.scrollLeft += 300);
...
tabsPersonal.forEach((tab, index) => {
  ...
  const arrowPersonal = $$(
    ".container__personal--control .personal__title--right"
  );
  tab.onclick = () => {
    if (index == 0) {
